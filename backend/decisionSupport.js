/**
 * Custom Feature: decision-support
 * Farmer decision-support recommendations
 *
 * POST /api/ai/decision-support
 * Auth required (inline JWT verification matching server.js convention).
 * Integration credentials: process.env.FEATURE_DECISION_SUPPORT_KEY
...
const FEATURE_KEY = process.env.FEATURE_DECISION_SUPPORT_KEY;
const SYSTEM_PROMPT = `You are an expert agricultural assistant specialized in: Farmer decision-support recommendations.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICropDiseasePestDetection - decision-support',
    },
    body: JSON.stringify({
...
router.post('/decision-support', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_DECISION_SUPPORT_KEY');
    const ai = await callLLM({ feature: 'decision-support', goal: 'Farmer decision-support recommendations', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          `INSERT INTO ai_results (user_id, endpoint, model, result) VALUES ($1, $2, $3, $4)`,
          [req.userId || null, 'decision-support', ai.model, JSON.stringify({ parsed: ai.result })]
        );
      }
    } catch (e) { console.warn('[decision-support] persistence skipped:', e.message); }
    return res.json({
      ok: true,
      feature: 'decision-support',
      endpoint: '/api/ai/decision-support',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[decision-support] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/decision-support/health', (req, res) => {
  res.json({
    feature: 'decision-support',
    endpoint: '/api/ai/decision-support',
