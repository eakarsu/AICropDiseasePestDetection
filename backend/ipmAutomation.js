/**
 * Custom Feature: ipm-automation
 * Integrated pest management automation
 *
 * POST /api/ai/ipm-automation
 * Auth required (inline JWT verification matching server.js convention).
 * Integration credentials: process.env.FEATURE_IPM_AUTOMATION_KEY
...
const FEATURE_KEY = process.env.FEATURE_IPM_AUTOMATION_KEY;
const SYSTEM_PROMPT = `You are an expert agricultural assistant specialized in: Integrated pest management automation.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AICropDiseasePestDetection - ipm-automation',
    },
    body: JSON.stringify({
...
router.post('/ipm-automation', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_IPM_AUTOMATION_KEY');
    const ai = await callLLM({ feature: 'ipm-automation', goal: 'Integrated pest management automation', input: payload });
    try {
      if (pool && pool.query) {
        await pool.query(
          `INSERT INTO ai_results (user_id, endpoint, model, result) VALUES ($1, $2, $3, $4)`,
          [req.userId || null, 'ipm-automation', ai.model, JSON.stringify({ parsed: ai.result })]
        );
      }
    } catch (e) { console.warn('[ipm-automation] persistence skipped:', e.message); }
    return res.json({
      ok: true,
      feature: 'ipm-automation',
      endpoint: '/api/ai/ipm-automation',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[ipm-automation] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/ipm-automation/health', (req, res) => {
  res.json({
    feature: 'ipm-automation',
    endpoint: '/api/ai/ipm-automation',
